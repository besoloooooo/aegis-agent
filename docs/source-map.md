...
Ports Hermes' FTS5-backed historical *session* search (not Auto Memory — this
queries the real chat log, distinct from the distilled long-term memory in
Stage 14/15).  Migrated as a whole unit: the FTS5 schema + trigger sync lives
in the existing `SQLiteSessionRepository` (no second session DB), the tool is
an adapted port of `tools/session_search_tool.py`.

| Aegis file | Relationship | Hermes source → symbol | Notes |
|---|---|---|---|
| `src/aegis_agent/sessions/sqlite_store.py` (FTS schema + sync) | **ADAPT** | `hermes_state.py` → `FTS_SQL` (376), `FTS_TRIGRAM_SQL` (405), `_FTS_TRIGGERS` (78), `_ensure_fts_schema` (618), `_rebuild_fts_indexes` (586), `_sqlite_supports_fts5` (556), `_drop_fts_triggers` (568), `_fts_trigger_count` (576) | `messages_fts` (unicode61/BM25) + `messages_fts_trigram` (CJK/substring). Triggers bind message INSERT/UPDATE/DELETE to the index in the same write txn (no "messages has data, FTS doesn't" split). Idempotent init; backfills from `messages` when the FTS table is freshly created or triggers were dropped by a prior no-FTS5 runtime. |
| `src/aegis_agent/sessions/sqlite_store.py` (`search_messages`, `get_messages_around`, `get_anchored_view`) | **ADAPT** | `hermes_state.py` → `search_messages` (3541), `_sanitize_fts5_query` (3458), `_contains_cjk`/`_count_cjk` (3522/3537), `get_messages_around` (2515), `get_anchored_view` (2593) | FTS5 `MATCH` + `snippet()` + BM25/`sort` ordering; CJK trigram path + short-CJK `LIKE` fallback; ±window + `bookend_start`/`bookend_end` + `messages_before`/`after`. Schema adaptations: `sessions.started_at` → `created_at`, `s.model` dropped (Aegis doesn't record it), `_decode_content` dropped (Aegis content is always a str), `active=1` filter added (Aegis soft-delete). |
| `src/aegis_agent/sessions/sqlite_store.py` (`list_sessions_rich`, `get_session_dict`, `get_messages_dict`) | **ADAPT** | `hermes_state.py` → `list_sessions_rich` (1888), `get_session` (1642), `get_messages` (2475) | `list_sessions_rich` simplified: the recursive compression-chain CTE / child filtering / `parent_session_id` projection are dropped (Aegis never forks sessions); keeps the single-query preview + last_active subqueries. `get_session_dict` aliases `started_at`←`created_at`, `model`←None. |
| `src/aegis_agent/tools/builtin/session_search.py` | **ADAPT** | `tools/session_search_tool.py` (whole file) | Single-shape tool, four calling modes (DISCOVERY/SCROLL/READ/BROWSE) inferred from args. Dropped: `profile` param, `_resolve_profile_db`/`_locate_session_db` cross-profile reads, `@session:<profile>/<id>` autosplit, compression-lineage `_resolve_to_parent` (identity), lineage-rebind in scroll. Kept: query semantics, FTS5 behavior, BM25/sort, ±window, bookends, tool params/return envelope/error handling. `db` is the injected `SQLiteSessionRepository`; unavailable (in-memory) store → `success=false` JSON, not a raise. |
| `src/aegis_agent/tools/schemas.py` (`SESSION_SEARCH`) | **ADAPT** | `tools/session_search_tool.py` → `SESSION_SEARCH_SCHEMA` (627) | Tool definition; `profile` param dropped (no profile subsystem). |
| `src/aegis_agent/tools/builtin/__init__.py` (`build_default_registry`) | **original** | — | Registers `SessionSearchTool` only when the injected `session_repository` exposes `search_messages` (SQLite/FTS5); in-memory store omits it. |
| `src/aegis_agent/tools/registry.py` (`ToolContext.session_id`), `tools/executor.py` (`session_id` injection), `runtime.py` (wiring) | **original** | — | Threads the active session id into `ToolContext` so `session_search` can exclude the current conversation (already in context), mirroring Hermes' `current_session_id` guard. Additive + backward-compatible. |
| `tests/test_session_search.py` | **ADAPT** | `tests/tools/test_session_search.py` (whole file) | Adapted for Aegis `SQLiteSessionRepository` (`create_session(sid)`, `append_message(sid, Message)`). Cross-profile tests dropped; FTS index-lifecycle tests (init/backfill/reopen/special-chars/empty) added. 49 tests. |

## Stage 17 — interactive slash-command suite (/save, /chatlog, /new, /undo, …)

Ports the interactive-CLI subset of Hermes' slash-command system.  Hermes'
central registry (`hermes_cli/commands.py`) also feeds gateway/Telegram/Slack
dispatch — Aegis keeps only the REPL-facing commands (gateway/product commands
are out of scope per CLAUDE.md §5).  The dispatcher is UI-agnostic (output via
an `emit` callable, session rotation via a callback) so it is testable without
Typer/Rich/a TTY, and the runtime never imports it.

| Aegis file | Relationship | Hermes source → symbol | Notes |
|---|---|---|---|
| `src/aegis_agent/slash_commands.py` (`CommandDef`, `COMMAND_REGISTRY`, `resolve_command`, `help_lines`) | **ADAPT** | `hermes_cli/commands.py` → `CommandDef` (line 45), `COMMAND_REGISTRY` (line 66), `resolve_command` (line 246) | Registry pattern + alias lookup retained; registry content reduced to the CLI subset (`new`/`clear`/`history`/`save`/`retry`/`undo`/`title`/`sessions`/`help`/`exit`). Gateway fields (`cli_only`/`gateway_only`/`gateway_config_gate`), prompt_toolkit completer/autosuggest, and the `utils.is_truthy_value` dep dropped. |
| `src/aegis_agent/slash_commands.py` (`SlashHandler._cmd_save` / `_dump_chatlog`) | **ADAPT** | `cli.py` → `HermesCLI.dump_chatlog` (line 7572) | `/save` implements Hermes' `/chatlog` behaviour (per user decision; `/chatlog` kept as an alias): the three-file `NN-local.json` / `NN-wire.json` / `NN-system.txt` dump with the scan-for-next-numeric-prefix scheme; output dir `aegis-chat-logs/` (cwd). System prompt prefers the captured wire copy, falls back to a fresh render (Hermes: `_cached_system_prompt` → rebuild). Hermes' separate `save_conversation` JSON export was NOT carried over. |
| `src/aegis_agent/slash_commands.py` (`WireCaptureProvider`) | **REWRITE** | `cli.py` → `HermesCLI._install_wire_capture` (line 7693, monkey-patches `agent._build_api_kwargs`) | Same observable capture (last outbound message list + timestamp, shallow list copy on the hot path) but as an explicit `ModelProvider` wrapper instead of a monkey-patch — the Aegis provider Protocol makes this a composition seam, not a patch. The CLI passes the *unwrapped* provider as the memory side-query backend so recall/extract calls never clobber the conversation capture. |
| `src/aegis_agent/slash_commands.py` (`SlashHandler._cmd_history`) | **ADAPT** | `cli.py` → `HermesCLI.show_history` (line 6828) | Same rendering: tool messages collapsed into a `(N tool messages hidden)` summary, 400-char previews, `(requested N tool calls)` for tool-only assistant turns. Reads the repository (source of truth) instead of an in-memory `conversation_history` list. |
| `src/aegis_agent/slash_commands.py` (`SlashHandler._cmd_retry` / `_cmd_undo`) | **ADAPT** | `cli.py` → `HermesCLI.retry_last` (line 7727), `undo_last` (line 7766) | Same walk-back-to-Nth-user-message truncation semantics; `/undo` prefills the composer via prompt_toolkit `default=`. Hermes' memory-provider notify + agent-surgery steps have no Aegis counterpart (omitted). Truncation goes through the new repository `rewind_from_seq` (below) because Aegis' source of truth is the store, not an in-memory list. |
| `src/aegis_agent/sessions/sqlite_store.py` (`rewind_from_seq`) | **ADAPT** | `hermes_state.py` → `SessionDB.rewind_to_message` (soft-delete rewind) | Soft delete (`active=0` — the column Stage 12's schema reserved for this) instead of physical delete: rows survive for audit; all read paths (replay/tail/count/search) already filter `active=1`. The same transaction recomputes `message_count` and bumps `history_version`, so pre-rewind snapshots fail version validation and resume falls back to full replay of the truncated history. |
| `src/aegis_agent/sessions/sqlite_store.py` (`set_session_title`), `sessions/memory_store.py` (`set_session_title`, `rewind_from_seq`) | **REWRITE** | `hermes_state.py` → `SessionDB.set_session_title`; `cli.py` → `/title` handler | Store-level title setter (the schema column existed since Stage 12). The in-memory store truncates physically (ephemeral; nothing to audit). |
| `src/aegis_agent/slash_commands.py` (`sanitize_title`) | **REWRITE** | `hermes_state.py` → `SessionDB.sanitize_title` | Same observable rules: printable-only, whitespace-collapsed, length-capped (60), empty → invalid. |
| `src/aegis_agent/cli.py` (REPL wiring, `_rotate_session`) | **original** | — | `/`-lines offered to `SlashHandler` first; unknown tokens fall through to skill routing then the model (pre-existing behaviour). `/new` rotates the cross-process lease via `SessionLeaseManager.switch_session` (acquire-new-then-release-old) and clears the terminal `lease_lost` marker, mirroring `cli.py:new_session`'s `_stop_session_lease` + `_lease_lost = False`. |
| `src/aegis_agent/runtime.py` (`provider_name`, `system_prompt` properties) | **original** | — | Read-only accessors so the command layer can label exports and dump the system prompt without reaching into privates. |
| `src/aegis_agent/tui.py` (`prompt(default=)`, `out`, `clear_screen`) | **original** | — | Composer prefill for `/undo` (prompt_toolkit `default=`); block-output and clear-screen seams for the handler. |
...
| `src/aegis_agent/memory/prompt.py` (`MEMORY_BEHAVIOR_GUIDANCE_PROJECT`, `MemoryBehaviorContributor(project=, project_root=, memory_dir_path=)`) | **ADAPT** | `src/memdir/memoryTypes.ts` behaviour text | Project-scope behaviour section: project memory is the active index, `USER.md` is still global, project facts belong here, plain `user` facts belong in `USER.md`. Also appends the concrete **project root** and the **memory directory** absolute paths (both scopes) — the hashed `<home>/projects/<id>/memory` location is unguessable, and without it the model resolves index links against the project root / cannot edit or delete memories when asked. |
...
| `src/aegis_agent/tui.py` (`_FullscreenShell`, `_HistoryControl`) | **REWRITE** | Claude Code `src/components/FullscreenLayout.tsx`, `src/components/ScrollKeybindingHandler.tsx`, `src/ink/components/ScrollBox.tsx`, `src/keybindings/defaultBindings.ts` (`ctrl+end` → `scroll:bottom`); Hermes `cli.py` fixed-bottom prompt layout | Adds a prompt_toolkit full-screen layout with a scrollable output pane and fixed one-row bottom composer. The 2026-08-29 repairs clamp tail-following to the real viewport, preserve position after manual scroll-up, record submitted user text, route tool/error output through the history buffer, and explicitly forward VT100 wheel events from the inner history control to the outer `ScrollablePane`. Wheel movement uses Claude Code's precision baseline of one row per event, and `Ctrl+End` mirrors Claude Code's jump-to-bottom binding while restoring sticky follow. Viewport clipping keeps an absolute `_history_scroll` separate from the clipped pane's local `vertical_scroll`, preventing long live streams from freezing a manually scrolled view or hiding the thinking status. The misleading, non-draggable slice-local scrollbar is hidden. It does not port Claude Code's acceleration, DOM/Ink ScrollBox, virtual-scroll implementation, or Hermes' full CLI layout. |
...
| `tests/test_tui.py` | **original** | — | Adds coverage for Markdown rendering safety/ANSI styling, live Markdown before turn end, no empty frame between streaming deltas, one-row mouse-wheel scrolling, tail stickiness, clipped global-to-local scroll translation, long-stream position stability, thinking-status visibility, hidden scrollbar state, the `Ctrl+End` binding and restored follow mode, composer token classification, and slash hints while preserving existing event-order and CLI plain-output tests. |
